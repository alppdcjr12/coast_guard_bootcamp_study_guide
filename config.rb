...
RANDOM_CHAIN_OF_COMMAND_QUESTIONS = CHAIN_OF_COMMAND_QUESTIONS.map.with_index do |question, i|
  if i % 2 == 0 && i < CHAIN_OF_COMMAND_QUESTIONS.length-1
    [question, CHAIN_OF_COMMAND_QUESTIONS[i+1]].sample
  end
end

RANDOM_CHAIN_OF_COMMAND_QUESTIONS.reject! { |question| question.nil? }

SELECTED_QUESTIONS = RANDOM_CHAIN_OF_COMMAND_QUESTIONS +
                      ESSENTIAL_CORE_VALUES_QUESTION +
                      FIREARM_SAFETY_RULES_QUESTIONS +
                      GENERAL_ORDERS_QUESTIONS +
                      HISTORY_QUESTIONS[-5..-1] + HISTORY_QUESTIONS[..-5].sample(10)
                      USCG_ETHOS_QUESTION

ESSENTIAL_PLUS_RANDOM_SAMPLE = SELECTED_QUESTIONS + RANDOM_RATES_AND_RANKS_QUESTIONS
ESSENTIAL_PLUS_RANDOM_SAMPLE_PHONETIC = SELECTED_QUESTIONS + RANDOM_PHONETIC_RATES_AND_RANKS_QUESTIONS
...
SPECIAL_FOCUS_ARGS = {
  ...
  "ESSENTIAL_SAMPLE" => ESSENTIAL_PLUS_RANDOM_SAMPLE.shuffle,
  "ESSENTIAL_PHONETIC_SAMPLE" => ESSENTIAL_PLUS_RANDOM_SAMPLE_PHONETIC,
}
